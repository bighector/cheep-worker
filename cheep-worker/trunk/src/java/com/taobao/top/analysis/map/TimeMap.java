...
public class TimeMap implements IReportMap
{
			...
			Map<String, Alias> aliasPool,String tempMapParams)
	{
		...
		String key = ReportUtil.generateKey(entry, contents);

		if (AnalysisConstants.IGNORE_PROCESS.equals(key))
			return AnalysisConstants.IGNORE_PROCESS;
		
		long currentTime = Long.valueOf(contents[Integer.valueOf(entry.getKeys()[0]) - 1]);
		...
		if (entry.getKeys().length > 1)
		{
			...
			{
				otherkeys.append("--").append(
						contents[Integer.valueOf(entry.getKeys()[i]) - 1]);
			}
		}
		
		String mapParams = tempMapParams != null? tempMapParams:entry.getMapParams();
		...
		if (mapParams != null && !mapParams.equals(""))
		{
			...
			if (mapParams.startsWith("minute"))
			{
				...
				if (!mapParams.equals("minute"))
					interval = Integer.valueOf(mapParams.substring(
							"minute:".length()));
