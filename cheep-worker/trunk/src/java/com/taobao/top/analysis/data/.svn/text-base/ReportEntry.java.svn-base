...
public class ReportEntry implements Serializable,Cloneable
{
	...
	public void setValuefilter(String valuefilter)
	{
		...
		{
			String[] filters = valuefilter.split("&");
			...
			for (String f : filters)
			{
				...
				else
				{
					...
					else
					{
						...
						if (f
								.startsWith(AnalysisConstants.CONDITION_EQUALORGREATER)
								|| f
										.startsWith(AnalysisConstants.CONDITION_EQUALORLESSER)
								|| f
										.startsWith(AnalysisConstants.CONDITION_NOT_EQUAL))
						{
						...
						else
						{
							if (f.startsWith(AnalysisConstants.CONDITION_EQUAL)
									|| f
											.startsWith(AnalysisConstants.CONDITION_GREATER)
									|| f
											.startsWith(AnalysisConstants.CONDITION_LESSER))
							{
	...
	public void setConditions(String conditions, Map<String, Alias> aliasPool)
	{
		...
		if (conditions != null && !"".equals(conditions))
		{
			...
			for (String con : cons)
			{
				...
				String key = ReportUtil.transformVar(con.substring(1, con
						.lastIndexOf("$")), aliasPool);
	...
	public void appendConditions(String conditions, Map<String, Alias> aliasPool)
	{
		...
		this.setConditions(conditions, aliasPool);
